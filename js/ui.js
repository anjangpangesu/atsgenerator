/* Komentar Profesional:
   ui.js memegang logika utama untuk merender pembentukan dokumen HTML secara real-time.
   Pembaruan: Penambahan debounce auto-save dan modifikasi khusus modul Pendidikan (Education) 
   agar dapat menangani skala spesifik (IPK Universitas vs Nilai Sekolah Dasar/Menengah).
*/

const views = [
    "dashboard", "add-cv", "history-cv", "add-letter", "history-letter", 
    "add-leave", "add-resign", "history-other"
];
...
function createFieldHTML(type, id) {
    ...
    let nameFieldHTML = "";
    if (type === "education") {
        /* Input dinamis untuk IPK dan Nilai dikalibrasi menggunakan seleksi Dropdown */
        nameFieldHTML = `
        ...
        let p1 = type === "experience" ? "Nama Perusahaan" : type === "project" ? "Nama Proyek" : "Nama Organisasi";
        nameFieldHTML = `
    ...
    innerHTML = `
        <div class="grid grid-cols-1 md:grid-cols-4 gap-3 mb-2">
            ...
            <div class="col-span-1 md:col-span-4"><textarea class="w-full border rounded p-2 text-sm ${prefix}desc h-20" placeholder="Deskripsi Tugas/Aktivitas (Gunakan enter untuk point baris baru)" oninput="updateCVPreview()"></textarea></div>
        </div>`;
...
function getDynamicData(type) {
    ...
    container.querySelectorAll(".p-4").forEach((block) => {
        ...
        block.querySelectorAll("input, textarea, select").forEach((input) => {
            const cls = Array.from(input.classList).find((c) => c.startsWith(prefix));
            if (cls) {
...
function renderCategorizedList(title, data, type) {
    ...
    const skillOrder = ["Ahli (Expert)", "Mahir (Advanced)", "Menengah (Intermediate)", "Dasar (Beginner)", "Tanpa Keterangan"];
    const langOrder = ["Penutur Asli (Native)", "Fasih (Fluent)", "Profesional (Professional Working)", "Terbatas (Limited Working)", "Dasar (Elementary)", "Tanpa Keterangan"];
...
function updateCVPreview() {
    ...
    const address = document.getElementById("cv-address").value || "Kecamatan, Kota";
    ...
    if (contactLine2.length > 0) contactString += "<br>" + contactLine2.join(" | ");
    ...
    if (summary) {
        ...
        html += `<div class="ats-section-title">Ringkasan Profil</div><div class="ats-desc">${splitSummary}</div>`;
    }
    ...
    if (eduData.length > 0) {
        ...
        eduData.forEach((edu) => {
            ...
            // Format IPK atau Nilai secara dinamis
            if (edu.score || edu.gpa) {
                ...
                else if (edu.scale === 'nilai100') subInfo.push(`Nilai: ${scoreVal} / 100`);
            ...
            html += `
                ...
                <div class="ats-desc">${parseBullets(edu.desc)}</div>
            `;
        });
    }
    ...
    const renderSection = (title, data) => {
        ...
        data.forEach((item) => {
            ...
            secHtml += `
                ...
                <div class="ats-desc">${parseBullets(item.desc)}</div>
            `;
        });
        return secHtml;
    };
    ...
    if (certData.length > 0) {
        ...
        certData.forEach((cert) => {
            ...
            html += `
                ...
                <div class="ats-desc">${parseBullets(cert.desc)}</div>
            `;
        });
    }
    ...
    triggerAutoSaveCV(); // Memicu auto-save setiap ada perubahan UI
}
...
function buildSavedCVHTML(form) {
    ...
    if (form.header.photo) html += `<img src="${form.header.photo}" style="width: 90px; height: 120px; object-fit: cover;">`;

    let contactAlign = form.header.photo || form.header.qrcode ? "left" : "center";
    ...
    let contactFontSize = form.header.photo && form.header.qrcode ? '8.5pt' : '10pt';
    ...
    if (form.header.qrcode) html += `<img src="${form.header.qrcode}" style="width: 80px; height: 80px; object-fit: contain;">`;
    html += `</div>`;

    if (form.header.summary) {
        const splitSummary = form.header.summary.split("\n").filter((s) => s.trim() !== "").map((s) => `<p>${s}</p>`).join("");
        html += `<div class="ats-section-title">Ringkasan Profil</div><div class="ats-desc">${splitSummary}</div>`;
    }
    ...
    html += renderCategorizedList("Skill / Keterampilan", form.skillList || [], "skill");
...
function updateLetterPreview() {
    ...
    const attachment = document.getElementById("cl-attachment").value || "[Jumlah Lampiran]";
    const to = document.getElementById("cl-to").value || "[Nama Tujuan / HRD]";
    const companyStreet = document.getElementById("cl-company-street").value || "[Alamat Jl dan Nomor]";
    const companyCity = document.getElementById("cl-company-city").value || "[Kota/Kab]";
    const companyZip = document.getElementById("cl-company-zip").value || "[Kode Pos]";
    ...
    const address = document.getElementById("cl-address").value || "[Alamat Lengkap]";
    ...
    const p1Raw = document.getElementById("cl-p1").value || "[Paragraf Pembuka]";
    const p3Raw = document.getElementById("cl-p3").value || "[Paragraf Maksud & Kelebihan]";
    const p4Raw = document.getElementById("cl-p4").value || "Curriculum Vitae\nFotokopi Ijazah\nPortofolio";
    const p5Raw = document.getElementById("cl-p5").value || "[Paragraf Penutup]";

    let p1Lines = p1Raw.split("\n").filter((l) => l.trim() !== "");
    if (p1Lines.length > 0) {
        p1Lines[p1Lines.length - 1] += " Yang bertanda tangan di bawah ini:";
    } else {
        p1Lines.push("Yang bertanda tangan di bawah ini:");
    }
    const formattedP1 = p1Lines.map(line => `<p style="margin-bottom: 12px; text-align: justify;">${line}</p>`).join("");

    const p4ListItems = p4Raw.split("\n").filter((l) => l.trim() !== "").map((l) => `<li style="margin-bottom: 4px;">${l}</li>`).join("");
    const formattedP4 = p4ListItems ? `<ul style="margin-left: 20px; padding-left: 20px; list-style-type: disc; margin-bottom: 12px;">${p4ListItems}</ul>` : "";
...
function buildSavedLetterHTML(form) {
    ...
    let dateStr = "[Tanggal]";
    if (form.dateInput) {
        dateStr = new Date(form.dateInput).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
    }
    const headerDate = `${cityStr}, ${dateStr}`;
    const subject = form.subject || "[Perihal]";
    const attachment = form.attachment || "[Jumlah Lampiran]";
    const to = form.to || "[Nama Tujuan / HRD]";
    const companyStreet = form.companyStreet || form.companyAddr || "[Alamat Jl dan Nomor]";
    ...
    const p1Raw = form.p1 || "[Paragraf Pembuka]";
    const p3Raw = form.p3 || "[Paragraf Maksud & Kelebihan]";
    const p4Raw = form.p4 || "Curriculum Vitae\nFotokopi Ijazah\nPortofolio";
    const p5Raw = form.p5 || "[Paragraf Penutup]";
...
function updateLeavePreview() {
    ...
    let dateStr = "[Tanggal]";
    if (dateInputVal) {
        dateStr = new Date(dateInputVal).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
    }
    const headerDate = `${cityStr}, ${dateStr}`;

    const to = document.getElementById("lv-to").value || "[Nama Tujuan / Atasan]";
    const company = document.getElementById("lv-company").value || "[Nama Perusahaan]";
    ...
    const department = document.getElementById("lv-department").value || "[Divisi/Departemen]";
    
    let startStr = "[Mulai]";
    const startVal = document.getElementById("lv-start").value;
    if(startVal) startStr = new Date(startVal).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
    
    let endStr = "[Selesai]";
    const endVal = document.getElementById("lv-end").value;
    if(endVal) endStr = new Date(endVal).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
    
    const reason = document.getElementById("lv-reason").value || "[Alasan Cuti]";
    ...
    const html = `
        ...
        <div class="cl-body">
            ...
            <p style="margin-bottom: 12px; text-align: justify;">
                ...
                Adapun alasan dari pengajuan permohonan cuti ini adalah karena <b>${reason}</b>.
            </p>
            <p style="margin-bottom: 12px; text-align: justify;">
...
function buildSavedLeaveHTML(form) {
    ...
    let startStr = "[Mulai]";
    if(form.start) startStr = new Date(form.start).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
    let endStr = "[Selesai]";
    if(form.end) endStr = new Date(form.end).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
    const reason = form.reason || "[Alasan Cuti]";
...
function updateResignPreview() {
    ...
    const to = document.getElementById("rs-to").value || "[Nama Tujuan / Pimpinan]";
    const company = document.getElementById("rs-company").value || "[Nama Perusahaan]";
    ...
    const department = document.getElementById("rs-department").value || "[Divisi/Departemen]";
    
    let effectiveStr = "[Tanggal Resign]";
    const effectiveVal = document.getElementById("rs-effective").value;
    if(effectiveVal) effectiveStr = new Date(effectiveVal).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
...
function buildSavedResignHTML(form) {
    ...
    let effectiveStr = "[Tanggal Resign]";
    if(form.effective) effectiveStr = new Date(form.effective).toLocaleDateString("id-ID", { year: "numeric", month: "long", day: "numeric" });
...
function renderCVHistory() {
    ...
    const sortedCVData = [...cvData].sort((a, b) => new Date(b.date) - new Date(a.date));
...
function editCV(id) {
    ...
    document.getElementById("cv-summary").value = cv.form.header.summary;
    ...
    ["education", "experience", "project", "org", "cert"].forEach((type) => {
        ...
        if (cv.form[type] && cv.form[type].length > 0) {
            cv.form[type].forEach((item) => {
                ...
                document.getElementById(`container-${type}`).insertAdjacentHTML("beforeend", createFieldHTML(type, newId));
                ...
                Object.keys(item).forEach((key) => {
                    ...
                    /* Backward compatibility untuk form CV lama yang menggunakan input 'gpa' langsung */
                    if (key === 'gpa' && item[key]) {
                        const scoreInput = block.querySelector(`.${prefix}score`);
                        if(scoreInput && !item.score) scoreInput.value = item[key];
                    }
                });
            });
        } else {
...
function resetCVForm() {
    ...
    document.querySelectorAll("#view-add-cv input, #view-add-cv textarea, #view-add-cv select").forEach((el) => {
...
function renderLetterHistory() {
    ...
    const sortedLetterData = [...letterData].sort((a, b) => new Date(b.date) - new Date(a.date));
...
function editLetter(id) {
    ...
    document.getElementById("cl-company-street").value = cl.form.companyStreet || cl.form.companyAddr || "";
    ...
    document.getElementById("cl-p1").value = cl.form.p1;
    document.getElementById("cl-p3").value = cl.form.p3;
    document.getElementById("cl-p4").value = cl.form.p4;
    document.getElementById("cl-p5").value = cl.form.p5;
...
function resetLetterForm() {
    ...
    document.querySelectorAll("#view-add-letter input, #view-add-letter textarea").forEach((el) => (el.value = ""));
    const indicator = document.getElementById("cl-autosave-indicator");
    if(indicator) indicator.innerText = "";
    updateLetterPreview();
}
...
function renderOtherHistory() {
    ...
    const sortedData = [...otherLetterData].sort((a, b) => new Date(b.date) - new Date(a.date));
    sortedData.forEach((item) => {
        const tr = document.createElement("tr");
        tr.className = "border-b hover:bg-gray-50";
        tr.innerHTML = `
...
function editOtherLetter(id) {
    ...
    if (item.type === 'leave') {
        ...
        document.getElementById("lv-reason").value = item.form.reason || "";
        ...
        document.getElementById("rs-reason").value = item.form.reason || "";
...
function resetLeaveForm() {
    ...
    document.querySelectorAll("#view-add-leave input, #view-add-leave textarea").forEach((el) => (el.value = ""));
...
function resetResignForm() {
    ...
    document.querySelectorAll("#view-add-resign input, #view-add-resign textarea").forEach((el) => (el.value = ""));
